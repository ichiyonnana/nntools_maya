...
def orig_point_from_vertex(vtx):
    """ [cmds] maya.cmds のコンポーネント文字列や MeshVertex からデフォーム前の座標を取得する
    ...
    """
    obj = nu.get_parent(nu.get_object(vtx))
    index = nu.get_indices(vtx)[0]
    orig_shape = [x for x in cmds.listRelatives(obj, noIntermediate=False) if cmds.getAttr(x + ".intermediateObject")][0]
    vtx = f"{orig_shape}.vtx[{index}]"
...
def set_vtx_weight(sc: str, vtx: str, weight_dic: dict):
    """辞書でウェイト設定
...
def _linearize_weight(target: str, end_vts_weights: list[dict[str, float]], end_vts_points: list[list[float, float, float]]):
    """ 指定した代表点のウェイトと座標で指定した対象頂点のウェイトをリニアにする

    Args:
        target (str): 操作対象頂点
        end_vts_weights (list[dic[str, float]]):
...
def avarage_weights(weights: list[dict[str, float]], alpha_list: list[float]):
    """ ウェイトの加重平均を求める
...
def linearize_weight_with_borders(vts):
    ...
    """
    internal_faces = cmds.filterExpand(cmds.polyListComponentConversion(vts, fv=True, tf=True, internal=True), sm=34)
    border_edges = cmds.filterExpand(cmds.polyListComponentConversion(internal_faces, ff=True, te=True, border=True), sm=32)
...
def smooth_weight_linear(keep_zero=False):
    ...
    internal_faces = cmds.filterExpand(cmds.polyListComponentConversion(target_vertices, fv=True, tf=True, internal=use_internal_border), sm=34)
    border_edges = cmds.filterExpand(cmds.polyListComponentConversion(internal_faces, ff=True, te=True, border=True), sm=32)
...
def smooth_weight_interior(target_vertices=None, keep_zero=True):
    ...
    if len(border_groups) == 1 or True:
        ...
        connected_vertex_indices = [nu.get_indices(vtx)[0] for target_vertex in target_vertices for vtx in nu.get_connected_vertices(target_vertex)]
        border_vertex_indices = [nu.get_indices(vtx)[0] for border_group in border_groups for edge in border_group for vtx in nu.pynode(nu.to_vtx(edge))]
        end_vertex_indices = set(border_vertex_indices) & set(connected_vertex_indices)
        end_vertices = [f"{obj}.vtx[{i}]" for i in end_vertex_indices]
